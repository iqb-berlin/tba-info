...
Hier wird der Großteil der zur Verfügung stehenden [Booklet-Konfigurations-Parameter](https://pages.cms.hu-berlin.de/iqb/testcenter/pages/booklet-config.html#list-of-parameters) geprüft. Die hier aufgeführten Attribute und Werte können innerhalb des Booklets im Feld: `<BookletConfig>` angegeben werden. Um den Aufbau dieser spezifischen Tests etwas verständlicher zu machen, erfolgt an dieser Stelle eine kurze Erläuterung, wie die Tests organisiert sind. Während der Testung werden die Werteoptionen (**default** - **value 1-3**) in der nachfolgenden Tabelle für jeden Parameter getestet.
...
```
Im Verzeichnis: **e2e/src/e2e/Booklet-Config** sind die dafür verwendeten Testdateien zu finden. Die Bezeichner entsprechen dabei den möglichen Werteoptionen (**default** - **value-3**). Zur Durchführung dieser Tests werden Booklets im Verzeichnis **sampledata/system-test** eingebunden. Der Bezeichner dieser Booklets enthält ebenfalls eine der möglichen Werteoptionen.

Es gibt weitere Konfigurationsmöglichkeiten in der Booklet-XML, die nicht im Feld `<BookletConfig>` angegeben sind, sondern Testlet spezifisch agieren.
Diese sind:
...
<table>
    ...
    <tbody>
        <tr>
            <td align="left"><code>DenyNavigationOnIncomplete</code></td>
        ...
        <tr>
            <td align="left"><code>LockAfterLeaving</code></td>
            <td align="left"></td>
...
Die Standardwerte dieser Attribute werden bereits mit dem Testen der Attribute aus der oberen Tabelle abgedeckt. Alle anderen Werte werden 
in der Testdatei: **TestletRestrictionParam.cy.ts** im Verzeichnis: **e2e/src/e2e/Booklet-Config** getestet. Das für diese Tests verwendeten Booklets tragen die Namen: **CY_BKL_TestletRestrictionParam_1.xml** und **CY_BKL_TestletRestrictionParam_1.xml**. Diese sind im Verzeichnis: **sampledata/system-test** zu finden.
