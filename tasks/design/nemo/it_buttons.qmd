...
* `textPosition`: Man kann über die Konstanten `ABOVE_IMAGE`, `ABOVE_BUTTONS` und `BOTTOM` (Standard) die Position des Textes beeinflussen.
...
```
1. `buttonType` legt Form, Größe und teilw. Position fest. Es sind Quadrate in verschiedenen Größen (`BIG_SQUARE`, `MEDIUM_SQUARE` Standard, `SMALL_SQUARE`), Text (`LONG_TEXT`, `SHORT_TEXT`) und `CIRCLE` als runder Button möglich.
