...
* **Testheft-Konfiguration:** Konfiguration Testverhalten, Testlayout, Testmethode

::: {.callout-important}
Die in der XML-Datei angelegten Attribute und Elemente sind nicht frei wählbar, sondern orientieren sich an einer Schema-Definition. Diese gibt vor, welche Elemente und Attribute verwendet werden dürfen. Die Schema Definition für die Testtaker-XML befindet sich im Testcenter GitHub-Repository [hier](https://github.com/iqb-berlin/testcenter/blob/master/definitions/vo_Booklet.xsd).
:::
...
```
::: {.callout-note}
In den nachfolgenden Code-Beispielen ist der Pfad zur Schema-Definition im Sinne der Übersichtlichkeit nicht immer enthalten!
:::
...
Die verfügbaren Attribute, die zugehörigen möglichen Attributwerte in der Booklet-XML und deren Bedeutung werden nachfolgend aufgelistet. Es wird auch aufgezeigt, welchen Elementen Daten in Form von Text zugeordnet werden können. Nicht alle Attributwerte sind frei wählbar, manche müssen entsprechenden Vorgaben entsprechen. **Die Attribute, Werte und Daten sind in der nachfolgenden Liste bis in die zweite Rekursionsebene gelistet und beschrieben.**
...
Es ist möglich Testhefte individuell zu konfigurieren. Hierfür steht ein Attribut: `key` im Unterlement: `Config` zur Verfügung. Diesem Attribut können vordefinierte Werte und Daten zugewiesen 

::: {.callout-note}
[Hier](https://pages.cms.hu-berlin.de/iqb/testcenter/pages/booklet-config.html) ist eine Liste der vordefinierten Attribute und Daten zu finden.
:::
