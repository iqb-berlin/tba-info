...
```
::: {.callout-important}
Die Werte für das Attribut `key` sind nicht frei wählbar. Diese sind einer entsprechenden Liste zu entnehmen. Bei Verwendung eines Listenwertes ist die Groß-/Kleinschreibung zu beachten. Alle Texte, die nicht geändert werden, erhalten einen Standardtext, der ebenfalls der Liste zu entnehmen ist.
:::

::: {.callout-note}
[Liste der Textersetzungen](https://pages.cms.hu-berlin.de/iqb/testcenter/pages/custom-texts.html)
:::

# Gruppenmonitoring

Der Gruppenmonitor kann in der Studien-XML über spezifische Modi eingeschaltet werden. Es stehen zwei Modi dafür zur Verfügung: `monitor-group` und `monitor-study`. Im Modus: `monitor-group` können alle Logins der zugehörigen Gruppe gesteuert werden. Im Modus: `monitor-study` können alle Logins in allen Gruppen gesteuert werden.
...
In diesem Beispiel wird ein Gruppenmonitor für die Gruppe: **sample_group** angelegt. Nach der Anmeldung am Testcenter mit dem Namen: **test-group-monitor** und dem Passwort: **er45tz** können die beiden Testhefte: **BOOKLET.SAMPLE-1** und **BOOKLET.SAMPLE-3** zur Voransicht gestartet werden. Außerdem ist eine Schaltfläche zu sehen, die den Gruppenmonitor startet. Der Gruppenmonitor zeigt den Status der beiden angelegten Personen (Codes): **xxx** und **yyy** und die Testheftstrukturen der zugeordneten Testhefte: **BOOKLET.SAMPLE-1** und **BOOKLET.SAMPLE-3** an.