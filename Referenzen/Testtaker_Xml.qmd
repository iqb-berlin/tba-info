...
Was ist in diesem Beispiel zu sehen und was würde passieren, wenn diese Datei mit den hier deklarierten Testheften und zugehörigen Aufgaben in das Testcenter geladen wird?
...
Die verfügbaren Attribute und die zugehörigen möglichen Attributwerte in der Testtaker-XML und deren Bedeutung werden nachfolgend aufgelistet. Außerdem wird augelistet welchen Elementen Daten in Form von Text zugeordnet werden können. **Die Attribute und Werte werden dabei dem jeweiligen Xml-Element zugeordnet.**

::: {.callout-important}
Nicht alle Attributwerte eines Attributs sind frei wählbar. Bei einzelnen können nur vordefinierte Werte zugewiesen werden. Zum Beispiel kann das Attribut: `mode` im Element: `<Login>` nur einen definierten Wert erhalten, der einer entsprechenden Liste zu entnehmen ist. Hingegen ist der Wert des Attributs: `name` in `<Login>` frei wählbar. Auch Dateneingaben (Texteingaben) sind nicht immer frei wählbar. Bspw. können dem Element `<Booklet>` Daten hinzugefügt werden, die bestimmen welches Booklet aufgerufen wird. Hier muss natürlich die richtige ID des Booklets eingetragen werden. Ob ein freier Wert hinterlegt werden kann oder nicht, ist den nachfolgenden Beschreibungen zu entnehmen.
:::
