...
Die Festlegung erfolgt in der XML zur Studien-Definition im Element `Login` mit Hilfe des Attributs `mode` und einem zugehörigen Wert. Nachfolgend ist ein einfaches Beispiel zu sehen:

::: {.callout-important}
Die Werte des Attributs: `mode` sind nicht frei wählbar. Sie sind einer entsprechenden Liste zu entnehmen. Es ist die Groß-/Kleinschreibung zu beachten bei Übernahme eines Listenwertes.
:::
...
In diesem Beispiel wird dem ersten Login: **me4r5t6** der Modus: `run-review` zugewiesen. Nach Anmeldung der Testperson das Booklet: **Booklet_Version1** präsentiert. Wie dem Namen schon zu entnehmen ist, werden bei diesem Modus spezielle Funktionen der Vorbetrachtung bereitgestellt. Die angemeldete Person kann bspw. über eine Kommentarfunktion entsprechende Kommentare zu den Aufgaben hinterlegen. Diese Kommentare können anschließend heruntergeladen werden. Aufgabenentwickler\*innen können diese dann zur Aufgabenoptimierung vor finaler Studiendurchführung verwenden.
...
[Liste möglicher Modi](https://pages.cms.hu-berlin.de/iqb/testcenter/pages/test-mode.html)
