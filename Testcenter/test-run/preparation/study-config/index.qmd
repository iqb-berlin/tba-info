...
Es ist unerlässlich die erstellte Konfiguration ausreichend zu testen. Hierfür können im Vorfeld spezielle Modi, wie bspw. der Modus: `run-simulation`, `run-demo` oder `run-review` in der Testtaker-XML zu einem Probelogin angelegt werden. Nach dem Login startet das Testcenter dann in einem dieser Probemodi.
...
::: {.callout-tip}
Es sollte auch einmal mit einem "heißen" Modus (`run-hot-restart`, `run-hot-return`) geprüft werden, ob Antwortdaten im Testcenter erzeugt werden.
:::