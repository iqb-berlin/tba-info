...
TLS Zertifikate können dabei eigenhändigt gepflegt werden oder mithilfe eines sog. ACME Anbieters (z.B. Let's Encrypt oder Sectigo) eingerichtet werden. Wird ein ACME Anbieter genutzt, so werden alle notwendigen Informationen im Installationsprozess abgefragt. 
Werden die Zertifikate manuell gepflegt, so sind der Public Key als `certificate.pem` und der Private Key als `private_key.pem` im Ordner `secrets/traefik/certs` abzulegen.
