...
format:
  html:
    code-overflow: wrap
    other-links:
      - text: XML-Spezifikation für Booklet
...
2. Metadaten: Die ID und das Label sind zwingend zu deklarieren. Die ID wird referenziert in den Testtaker-XML und das Label wird der Testperson angezeigt nach dem Login, wenn mehrere Testhefte zur Auswahl stehen.
...
Um die Erscheinung und das Verhalten des Booklets zu beeinflussen, gibt es eine Vielzahl von Parametern. Sie werden in einem dafür reservierten Abschnitt der Booklet-XML deklariert.
...
Im Testcenter können aber für Units in Blöcken bestimmte Restriktionen gesetzt werden, die sehr wichtig aus Sicht der Testadministration sind. Das folgende Beispiel enthält alle drei derzeit möglichen Restriktionen:
...
```
1. Die `id` ist für interne Zwecke notwendig und muss einmalig im Booklet sein. Das `label` kann z. B. auf einer erweiterten Navigationsseite verwendet werden.
2. Das Restrictions-Element muss stets das erste innerhalb eines Blockes sein. Die Elemente darin sind optional, müssen aber -- sofern vorhanden -- in genau dieser Reihenfolge vorkommen.
...
4. TimeMax: Nach Ablauf der angegeben Zeit wird in die erste Unit nach dem Block navigiert. Die Testperson kann das nicht verhindern. Es gibt für den Versuch des vorzeitigen Verlassens des Blockes ein optionales Attribut `leave`, das festlegt, ob eine Warnung ausgegeben wird (Wert `confirm`; Standardverhalten) oder ein Verlassen generell nicht erlaubt wird (Wert `forbidden`).
