...
Nach der Installation ist zunächst ein Administrator\*in-Konto für die Verwaltung von Arbeitsbereichen und Zugangsberechtigungen angelegt, der sogenannte **Super-Admin**. Außerdem ist bereits ein Arbeitsbereich angelegt mit Dateien für einen Beispieltest. Im ersten Schritt legt der Super-Admin Zugänge für einzelne Personen an und weist diesen Arbeitsbereiche zu. In den Arbeitsbereichen können dann, die für eine Studie benötigten Testdateien geladen werden. Hierfür steht eine entsprechende Verwaltungsoberfläche zur Verfügung, die die entsprechenden Bedienelemente zum laden, löschen usw. bereitstellt. Nach Abschluss einer Studie können über diese Verwaltungsoberfläche die gegebenen Antworten in Form einer CSV-Datei heruntergeladen werden.
...
# Systemanforderungen Studiendurchführung {Test}
...
Neu veröffentliche Browserversionen können unter Umständen Änderungen enthalten, die zu einem anderen Verhalten der Anwendung führen. Das IQB ist daher bemüht die Anwendung zeitnah in Verbindung mit Neuerscheinungen zu testen, kann dies aber nicht in jedem Fall gewährleisten, da die Vielzahl an möglichen neuen Browserversionen (Safari, Edge, Chrome, Firefox) recht herausfordernd ist. Sollten bereits neuere Versionen als die hier genannten Versionen zum Einsatz kommen, empfiehlt sich die Voransicht einer geplanten Studie vor finalem Studienlauf. Hierfür bietet das Testcenter spezielle Modi der Studiendurchführung an, wie bspw. einen Review-Mode. Außerdem kann auch der **System-Check** verwendet werden. 

Ist von einem fehlerhaften Verhalten der Anwendung in Verbindung mit neueren Browserversionen auszugehen, kann ein entsprechender Hinweis über die Support-Adresse: [iqb-tbadev-support@hu-berlin.de](mailto:iqb-tbadev-support@hu-berlin.de) erfolgen.

Es kann auch im Testcenter Repository bei GitHub ein entsprechendes Ticket angelegt werden. Diese wird dann direkt von den Programmierer\*innen bearbeitet. Hierfür ist eine Registration bei GitHub erforderlich.
