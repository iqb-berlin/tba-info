...
::: {.callout-tip}
Texte werden mit Hilfe bestimmter Attribute und Daten in der XML geändert. Hierfür gibt es eine spezielle Liste mit möglichen Werten, die im Element: `<CustomText>` und dem Attribut: `key` hinzugefügt werden können. [Link zur Liste](https://pages.cms.hu-berlin.de/iqb/testcenter/pages/custom-texts.html). Werde Texte nicht individuell ersetzt, erhalten sie eine Standardtext. Welcher Text das ist, ist der Liste zu entnehmen.
:::

Bestimmte Texte in der Anwendung und während des Studienlaufs können individuell angepasst werden. Im Vorfeld kann überlegt werden, an welchen Stellen keine Standardtexte stehen sollen. Hierfür kann einer Liste entnommen werden, welche Standardtexte an welchen Stellen zum Einsatz kommen. Ausgewählte können dann geändert werden.
...
![Bild 3: Anzeige Aufgabentitel, Ansicht Navigationsleiste und Schalter Navigationsmenu](TC_UnitTitle_NaviMenu.png)
...
* **Zeitbegrenzung:** Die Aufgaben des Block sind nur eine bestimmte Zeit zur Bearbeitung zu sehen. Nach Ablauf dieser Zeit, wird der Block gesperrt. Möchte die Testperson den Block vor Ablauf der Zeit verlassen, erscheint eine Meldung, dass nicht in den Block zurückgekehrt werden kann. Wird die Meldung bestätigt, wird der Block verlassen und gesperrt.

* **Beschränkungen Navigation:** Unabhängig von eventuell konfigurierten Navigationsbeschränkungen in der Testheft-Konfiguration (`BookletConfig`), können Navigationsbeschränkungen auch für ein Testlet festgelegt werden. Die konfigurierten Beschränkungen im Testlet überschreiben dabei die konfigurierten Beschränkungen in der Testheft-Konfiguration. Außerhalb des Testlets gelten dann wieder die Beschränkungen in der Testheft-Konfiguration.
...
Hier kann keine generelle Empfehlung erfolgen, weil es im hohen Maß vom Umfang einer Studie und auch von den Erwartungen an eine Studie abhängt,wie das Testheft aufgeteilt wird. Im Vorfeld sollten sich die Verantwortlichen eine konkrete Vorstellung davon machen, wie ihr Testheft aussehen soll. Nachfolgend ein paar gedanklichen Anregungen, die bestimmen würden, wie ein entsprechendes Testheft bzgl. Anforderungen strukturiert werden müsste:
...
* Nehmen an der Studie viele Personen teil mit unterschiedlichen Niveau, kann es sinnvoll sein Testlets mit Zugangsbeschränkungen zu versehen. Mit Hilfe dieser Freigabewörter können die Personen synchronisiert werden. Das verhindert, dass einzelne Personen schon mit der Studie durch sind, während andere noch mitten drin stecken.

* Soll ein Gruppenmonitor zur Steuerung einer Studie zum Einsatz kommen, sollten Testlets angelegt werden. Mit Hilfe des Gruppenmonitors kann dann die Testleitung die Testpersonen direkt zu einzelnen Testlets leiten.

Das soll an dieser Stelle erst einmal ausreichend sein. Es gebe hier unzählige weitere individuelle Anforderungen an eine Studie, die eine bestimmte der Testhefte erfordern würden.
