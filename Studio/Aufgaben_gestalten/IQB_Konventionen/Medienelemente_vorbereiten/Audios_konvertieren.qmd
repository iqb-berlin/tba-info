...
Die Audio-Datei kann entweder eine vorhandene Datei sein oder eine Audio sein, die mit KI (FreeTTS) generiert wurde (siehe unten).

::: {.callout-note}
Die Datei sollte eine MP3 sein, da es bei anderen Dateiformaten zu verlängertem Laden kommen kann. Dies kann mit Audacity angepasst werden (siehe unten).
:::
...
Bei dem Einfügen von Audio-Datein sind gewisse Einstellungen voreingestellt. Diese sind an den Einstellungen eines Stimulus-Audio orientiert: 

So z.B.
* Voreingestellte Lautstärke: 0,8 / Minimale Lautstärke 0,2
* Ggf. Anzahl der min./ max. Durchläufe anpassen
* Als Aufforderungstext zum Starten erscheint „Bitte starten” mit einer Verzögerung von 5000 sms
...
Gegebenfalls kann die Datei hier noch geschnitten werden und dann gespeichert werden. Dazu muss es exportiert werden:

![](Grafiken/Audacity_exportieren.png)

::: {.callout-important}
Beim Exportieren muss die Audiooption von Mono auf Stereo umgestellt werden.
:::