...
```
Jede Zeile beginnt mit der Id der Gruppe, dem Benutzernamen (loginname), dem Testheftnamen (bookletname) und dem Aufgabennamen (unitname). Im Anschluss folgt ein Antwortfeld (responses). In diesem Antwortfeld sind die Id's aller Aufgabenelemente und die zugehörigen Werte aufgelistet, die sich in der Aufgabe befinden. Die Struktur des Antwortfeldes sieht für jedes Aufgabenelement so aus:
