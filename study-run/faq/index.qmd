...
Eine Empfehlung welche Informationen benötigt werden, ist [hier](/study-run/run/support.qmd) zu finden.
...
* Zwischenspeicher (Cache) des Browsers löschen: Manchmal speichert der Browser Teile der Programmierung. Sollte aber auf dem Server eine neue Version dieser Programmierung vorliegen, muss diese unbedingt geladen werden - sonst funktioniert die Kommunikation mit dem Server nicht.
...
Es gibt da derzeitig keine Vorgaben seitens des IQB. Empfohlen werden die gängigen Browser, wie Google Chrome oder Firefox. **Mehr Information zu den Browser Anforderungen sind [hier](../preparation/required/school.qmd#software) zu finden.**
...
In der Dateiansicht eines Arbeitsbereichs wird die Testheftgröße angezeigt. Um zu verstehen, wie sich diese Größe bildet, müssen die Bestandteile eines Testheftes verstanden werden. Hierfür werden en unterteilt in:
...
# Bei der Wiedergabe einer Aufgabe kommt die Meldung: Unit-Definition veraltet

Editor-/ und Playerversion gehören immer zusammen. Bei der Aufgabenerstellung im Studio erzeugt der Editor eine Versionsnummer in der Unit-VOUD. Wird in der Unit-XML die angegebene Playerversion erhöht, ist es möglich diese höhere Version des Players zusammen mit der Aufgabe in das Testcenter zu laden. Beim Start des PLayers gleicht dieser aber seine Version mit der Version der Unit-VOUD ab. Gibt es hier eine Diskrepanz, erscheint eine Meldung, dass die Unit-Definition veraltet ist. Lösung: Soll ein neuerer Player verwendet werden, ist die Version im Studio für die jeweilige Aufgabe umzustellen und anschließend zu speichern. Im nächsten Schritt kann die Aufgabe exportiert werden und in das Testcenter geladen werden.

# Kann ich auf einem iPad die Safari Entwicklerkonsole nutzen?

Leider bietet Apple auf dem iPad keine Möglichkeit, die Safari Entwicklerkonsole zu nutzen. Es gibt aber eine Möglichkeit, die Entwicklerkonsole über ein Mobile-DevTools-Paket zu nutzen. Mehr dazu ist dem Kapitel: [Debugging](/study-run/faq/debugging.qmd) zu entnehmen.