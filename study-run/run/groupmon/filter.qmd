...
Es ist möglich mithilfe der {{< fa plus >}}-Schaltfläche weitere individuelle Filter einzurichten. Im erweiterten Modus kann auf reguläre Ausdrücke gefiltert werden. Im Standard-Modus kann auf "gleich" und "enthält" gefiltert werden. Wird ein adaptives Testheft verwendet, kann auf die dort hinterlegten State-Elemente und auf die zugehörigen Options-Elemente gefiltert werden. 

# Filter-Menü zur Einrichtung eines Filters

Nachfolgend ist das Filter-Menü zu sehen:
...
| Attribut | Feld im Menü | Beschreibung |
|----------|-------------|------------|--------------|
| `label` | Filtername | Anzeigebezeichnung für den Filter |
| `field` | Feld | Feld, auf das der Filter angewendet wird |
| `type` | Filtertyp | Art des Filters |
| `value` | Filter* | Filterwert |
| `not` | nicht | Negiert den Filter (invertierte Logik) |
...
Es folgt eine detaillierte Beschreibung der möglichen Attribute und der zulässiger Werte:
...
- **Typ:** `xs:string`
- **Erforderlich:** Nein
- **Beschreibung:** Anzeigebezeichnung für das Profil. Nach der erfolgreicher Gruppenmonitor Anmeldung wird diese Bezeichnung dem Benutzer zur Auswahl angezeigt. Im oberen Beispiel zu sehen mit den Labels: "Nicht alles zeigen" und "Alles zeigen".
...
- **Beschreibung:** Steuert die Filterung von ausstehenden (pending) Tests. Hierbei handelt es sich um einen der oben genannten Standardfilter. Dieser ist dann je nach Wert aktiv oder eben nicht.
...
- **Beschreibung:** Steuert die Filterung von gesperrten Tests. Hierbei handelt sich um einen der oben genannten Standardfilter. Dieser ist dann je nach Wert aktiv oder eben nicht.
