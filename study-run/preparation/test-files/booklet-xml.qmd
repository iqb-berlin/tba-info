...
4. **TimeMax**: Nach Ablauf der angegeben Zeit wird in die erste Unit nach dem Block navigiert und der Block wird gesperrt. Die Testperson kann das nicht verhindern. Die Testperson kann den Block auf Wunsch vor Ablauf der Zeit verlassen. Dazu navigiert die Person in eine Unit außerhalb des aktiven Blocks und erhält eine entsprechende Warnmeldung. Nach Bestätigung dieser, wird der Block frühzeitig verlassen und gesperrt. Mit Hilfe des Attributs: `leave` kann festgelegt werden, ob der Block verlassen werden darf. Hierfür stehen die Werte:  `allowed` und `forbidden` zur Verfügung. Der Wert: `confirm` legt fest, ob vor dem Verlassen eine Meldung angezeigt werden soll. 
